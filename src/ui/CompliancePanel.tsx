...
export function CompliancePanel() {
  ...
  const result = useMemo(() => {
    ...
    if (module) {
      const current = maxCircuitCurrent(module, 2)
      ...
      groups.push({
        title: 'Circuit current & conductors (NEC 690.8)',
        checks: [...current.checks, ...conductor.checks],
      })
    }
    ...
    if (acW > 0) {
      // Backfeed OCPD at 125% of continuous AC output, rounded to a standard size.
      const acAmps = acW / 240
      const breaker = Math.ceil((acAmps * 1.25) / 5) * 5
    ...
    if (cc && module && firstArray) {
      const r = sizeChargeController({
        module,
        modules_in_series: firstArray.rows,
        strings_in_parallel: firstArray.cols,
        record_low_temp_c: design.site.record_low_temp_c,
